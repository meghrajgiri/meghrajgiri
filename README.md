### Hi, I'm

# Meghraj Giri 👋

**Full Stack Developer | Building scalable web &amp; mobile products**

I build modern, fast, and scalable digital products that solve real-world problems.
With experience in web and mobile development, I turn complex ideas and business
requirements into clean, maintainable, and user-friendly applications.

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="assets/pills-dark.svg" />
  <img src="assets/pills-light.svg" width="430" alt="Nepal - meghrajgiri.com - 5+ Years of Experience" />
</picture>

### <img src="https://github.githubassets.com/images/icons/emoji/unicode/1f9e9.png?v8" width="20" alt="" /> What I Do

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="assets/what-i-do-dark.svg" />
  <img src="assets/what-i-do-light.svg" width="100%" alt="Web Applications: building modern, fast and responsive web applications. Mobile Applications: developing cross-platform mobile apps with great performance. Business Solutions: creating dashboards and management systems that solve real business needs. API Integration: working with REST and GraphQL APIs and third-party services. Scalable Architecture: writing clean, reusable and maintainable code at scale. User-Centric: building products with great user experience and accessibility." />
</picture>

### <img src="https://github.githubassets.com/images/icons/emoji/unicode/1f6e0.png?v8" width="20" alt="" /> Tech Stack

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="assets/tech-stack-dark.svg" />
  <img src="assets/tech-stack-light.svg" width="100%" alt="Frontend: React, Next.js, TypeScript, JavaScript, HTML5, CSS3, Tailwind CSS. Mobile: React Native, Expo, NativeWind. Backend and APIs: Node.js, NestJS, Express, GraphQL, REST APIs. Databases and Backend Services: PostgreSQL, MongoDB, Supabase, Firebase. Tools and Development: Git and GitHub, Docker, Nx Monorepo, Vercel, AWS." />
</picture>

### <img src="https://github.githubassets.com/images/icons/emoji/unicode/1f4c2.png?v8" width="20" alt="" /> Featured Projects

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="assets/projects-dark.svg" />
  <img src="assets/projects-light.svg" width="100%" alt="Clinic Management System - Cannabiz Elite, a clinic management system for Australian cannabis and telehealth clinics. React Native E-commerce App - Thriftverse, a thrift marketplace with unique storefront URLs. The LYVE App, a marketplace for event tickets and costume reservations. Inventory Management System - Khatapata, a PWA for small businesses." />
</picture>

<div align="center">

**[More projects on my portfolio →](https://meghrajgiri.com/projects)**

</div>

![Development Philosophy: I believe great software is built with clarity, purpose and responsibility. I focus on writing code that is maintainable, scalable, reusable, clean, user-focused, well-structured and performance-driven.](assets/philosophy.svg)

![Let's Connect: I'm open to collaborating on exciting projects and opportunities.](assets/connect.svg)

[![Portfolio - meghrajgiri.com](assets/btn-portfolio.svg)](https://meghrajgiri.com)&nbsp;&nbsp;&nbsp;[![LinkedIn - /in/meghrajgiri](assets/btn-linkedin.svg)](https://linkedin.com/in/meghrajgiri)&nbsp;&nbsp;&nbsp;[![Email - meghrajgiri56@gmail.com](assets/btn-email.svg)](mailto:meghrajgiri56@gmail.com)

<div align="center">

Thanks for visiting my profile! ⭐

</div>
