    ...
    <div className="columns-1 gap-4 sm:columns-2 lg:columns-3">
      {images.map((src, index) => {
        ...
        return (
          <div
            key={src}
            className="panel mb-4 block cursor-pointer break-inside-avoid overflow-hidden bg-muted"
...
export function ProjectDetail({
  ...
  return (
    <article className="px-6 pb-24 pt-10 md:pt-14">
      <div className="container mx-auto max-w-5xl">
        ...
        <div className="mb-12 space-y-6">
          <div className="flex flex-wrap items-center gap-3">
            <span className="rounded-[4px] bg-brand px-3 py-1 text-sm font-semibold text-brand-ink">
              {project.category}
            </span>
            <span
          ...
          <div className="flex flex-wrap gap-3">
            {project.links.demo && (
              <a
                ...
                className="cta focus-ring inline-flex min-h-[48px] items-center gap-2 px-6"
              >
                <svg
                  className="h-4 w-4"
                  fill="none"
            ...
            {project.links.github && (
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="cta-ghost focus-ring inline-flex min-h-[48px] items-center gap-2 px-6"
              >
                <svg
                  className="h-4 w-4"
                  fill="currentColor"
            ...
            {/* npm, like a store listing, is third-party proof: the package is
                published, versioned and installable by anyone. */}
            {project.links.npm && (
              <a
        ...
        {/* Hero Image */}
        <div
          className="panel mb-12 cursor-pointer overflow-hidden bg-muted"
        ...
        <div className="mb-12 grid gap-8 md:grid-cols-2">
          {/* Technologies */}
          <div className="panel space-y-4 bg-card p-6">
            <h2 className="text-lg font-semibold">Technologies</h2>
            <div className="flex flex-wrap gap-2">
              {project.technologies.map((tech) => (
                <span
                  key={tech}
                  className="rounded-[4px] border border-border bg-muted px-3 py-1.5 text-sm font-medium text-muted-foreground"
                >
                  {tech}
                </span>
              ))}
        ...
        {selectedImage && (
          ...
          >
            <button
              className="focus-ring absolute right-6 top-6 flex h-10 w-10 items-center justify-center rounded-full border border-white/25 bg-white/10 text-white transition-colors hover:bg-white/20"
              onClick={() => setSelectedImage(null)}
              aria-label="Close"
            >
              <svg
            ...
            {allImages.length > 1 && (
              <>
                <button
                  className="focus-ring absolute left-4 flex h-10 w-10 items-center justify-center rounded-full border border-white/25 bg-white/10 text-white transition-colors hover:bg-white/20"
                  onClick={(e) => {
                    e.stopPropagation();
                ...
                <button
                  className="focus-ring absolute right-4 flex h-10 w-10 items-center justify-center rounded-full border border-white/25 bg-white/10 text-white transition-colors hover:bg-white/20"
                  onClick={(e) => {
                    e.stopPropagation();
                    const currentIndex = allImages.indexOf(selectedImage);
