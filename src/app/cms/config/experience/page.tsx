...
export default function ExperienceConfigPage() {
  return (
    <ConfigEditor
      ...
      title="Experience"
      description="Work history shown in the Journey tab"
    >
      {({ config, updateField }) => {
        const experiences = config.experiences as Array<
        ...
        return (
          <div className="space-y-6">
            ...
            {experiences?.map((exp, i) => (
              <div
                key={i}
                className="relative space-y-4 rounded-[6px] border border-border p-4"
              >
                <div className="flex items-center justify-between">
                  <SectionLabel>
                    {(exp.title as string) || `Experience ${i + 1}`}
                  </SectionLabel>
                  <button
                ...
                <ArrayField
                  label="Technologies"
                  value={exp.technologies as string[]}
                  onChange={(v) => {
                    const e = [...experiences];
                    e[i] = { ...e[i], technologies: v };
                    updateField("experiences", e);
                  }}
                />
                <TextField
