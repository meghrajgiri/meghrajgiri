      ...
      {({ config, updateField }) => (
        <div className="space-y-6">
          ...
          {/* These drive the Person structured data. They used to be constants in
              ...
              places — and shipped `addressLocality: "Nepal"` for months as a result.
              "Location" above is the display string; these are the machine-readable
              parts. */}
          <SectionLabel>Search engine identity</SectionLabel>
          <p className="-mt-2 text-xs text-gray-500">
            Used in the Person structured data that tells Google and AI search which
            Meghraj this is. Locality means the city specifically.
          </p>
          <TextField label="City / locality" value={config.locality as string} onChange={(v) => updateField("locality", v)} />
          <TextField label="Region / province" value={config.region as string} onChange={(v) => updateField("region", v)} />
          <TextField label="Country code (e.g. NP)" value={config.country as string} onChange={(v) => updateField("country", v)} />
          <ArrayField
          ...
          {(((config.credentials as Array<Record<string, string>>) ?? [])).map((cred, i) => {
            const creds = (config.credentials as Array<Record<string, string>>) ?? [];
            ...
            return (
              <div key={i} className="space-y-4 rounded-lg border border-gray-200 p-4 dark:border-gray-800">
                <TextField label="Name" value={cred.name} onChange={(v) => set("name", v)} />
                <TextField label="Issued by" value={cred.issuer} onChange={(v) => set("issuer", v)} />
                <TextField label="Category" value={cred.category} onChange={(v) => set("category", v)} />
                <TextField label="URL (optional)" value={cred.url} onChange={(v) => set("url", v)} />
                <button
                  type="button"
                  onClick={() => updateField("credentials", creds.filter((_, j) => j !== i))}
